...
class App:
    ...
    def quadrant_window(self):
        ...
        # Crear el widget que contiene al plot
    ...
    def part_window(self):
        ...
        self.fig_2ax_2 = self.fig_2.add_subplot()
        ...
        toolbar = NavigationToolbar2Tk(self.canvas_part, miniframe).pack(fill='both', side='top', expand=True)
    ...
    def plot_part(self, all_parts, part_name):
        def pick_point(event, part):
            ...
            xvalue = np.take(xdata, ind)
            yvalue = np.take(ydata, ind)
            print(part.name, part.quadrant)
            if part.quadrant in ['Superior', 'Inferior']: # XY
                print('x:{} y:{}'.format(xvalue[0], yvalue[0]))
                clicked_point = self.find_pointXY(self.all_points, xvalue[0], yvalue[0])
            if part.quadrant in ['Derecho', 'Izquierdo']: # XZ
                print('x:{} z:{}'.format(xvalue[0], yvalue[0]))
                clicked_point = self.find_pointXZ(self.all_points, xvalue[0], yvalue[0])
            self.selected_registered_point_id = clicked_point.id
            self.entry6.config(text=self.selected_registered_point_id) # Point id
            self.entry4.config(text=self.selected_part) # Nombre de la parte
            self.entry8.config(text=part.quadrant) # Cuadrante de la parte
        ...
        for part in all_parts:
            if part.name == part_name and part_name != '-':
                ...
                if part.quadrant in ['Superior', 'Inferior']:
                    self.fig_2ax_2.plot(part.x, part.y, 'o', markersize=4, color='blue', picker=5)
                if part.quadrant in ['Derecho', 'Izquierdo']:
                    self.fig_2ax_2.plot(part.x, part.z, 'o', markersize=4, color='blue', picker=5)

                # Comando para seleccionar parte
                self.fig_2.canvas.mpl_connect('pick_event', lambda event: pick_point(event, found_part))
