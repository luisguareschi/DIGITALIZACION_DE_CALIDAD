...
class App:
    ...
    def draw_start_gui(self):
        ...
        self.defaultFont.configure(family='Cambria',
                                   size=14,
                                   weight=font.NORMAL)
    ...
    def select_record_window(self):
        ...
        # Titulo
        self.title1 = tk.Label(self.frame1, text='Airbus A350 XWB QD APP',font=('Cambria', '20'))
        self.title1.grid(row=0, column=0, sticky='ne', padx=20, pady=20)
        # Lista de archivos
        self.files_frame = tk.LabelFrame(self.frame1, text='Aircraft selection',font=('Cambria', '16'))
        self.files_frame.grid(row=1, column=0, columnspan=2, sticky='ewns', padx=20, pady=20)
        for i in range(0, 2):
        ...
        self.list1 = tk.Listbox(self.list_frame, bd=0, bg='#8BA9CB',font=('Cambria', '14'))
        ...
        self.button5 = ttk.Button(self.files_frame, text='New aircraft',
                                  command=lambda: [self.new_record_window()])
        self.button5.grid(row=0, column=1, sticky='s', padx=10, pady=20)
        # Boton de Start
        self.button6 = ttk.Button(self.files_frame, text='Start aircraft',
                                  command=lambda: [self.set_active_record(), self.change_state('LOADING')])
        self.button6.grid(row=0, column=0, sticky='s', padx=10, pady=20)
    ...
    def new_record_window(self):
        ...
        self.defaultFont.configure(family='Cambria',
                                   ...
                                   weight=font.NORMAL)
        # Frame
        self.frame3 = tk.LabelFrame(self.create_record_screen, text='Create new register',font=('Cambria', '14'))
        ...
        # Nombre del registro:
        self.label4 = tk.Label(self.frame3, text='Aircraft name:',font=('Cambria', '14'))
        self.label4.grid(row=0, column=0)
        # Entrada de texto
        self.entry3 = tk.Entry(self.frame3)
        self.entry3.grid(row=0, column=1)
        # Texto tipo de avion
        self.label5 = tk.Label(self.frame3, text='Aircraft version',font=('Cambria', '14'))
        ...
        self.button8 = ttk.Button(self.frame3, text='Create',
    ...
    def quadrant_window(self):
        ...
        self.button4 = ttk.Combobox(self.frame2, state='readonly',
                                    values=['Choose a quadrant', 'Upper', 'Lower', 'Right', 'Left'])
        ...
        miniframe = tk.LabelFrame(self.frame2, text='Quadrant')
        ...
        self.infobox = tk.LabelFrame(miniframe, text='Point information')
    ...
    def part_window(self):
        ...
        self.label2.grid(row=0, column=0, sticky='w',columnspan=4, pady=(0, 5), padx=(10, 10))
        # Separator
        self.separator = ttk.Separator(self.frame)
        self.separator.grid(row=1, column=0, columnspan=4, sticky='we', pady=(20, 30), padx=(10, 10))
        # Button de pieza/cuadrante
        self.button1 = ttk.Combobox(self.frame, state='readonly', values=['Part', 'Quadrant'], width=8)
        self.button1.grid(row=0, column=2, sticky='s', pady=(0, 20), padx=(0, 10))
        self.button1.bind("<<ComboboxSelected>>", self.set_config1)
        # Boton start
        self.button2 = ttk.Button(self.frame, text='View part',
                                  command=lambda: [self.set_part(),
                                                   self.plot_part(self.all_parts, self.selected_part)])
        self.button2.grid(row=0, column=1, sticky='s', pady=(0, 20))

        # Cuadro de partes
        self.labelframe1 = tk.LabelFrame(self.frame, text='Part name', font=('Cambria', '14'))
        self.labelframe1.grid(row=3, column=0, columnspan=2, sticky='ewn',pady=(0, 10), padx=(0, 10))
        for i in range(0, 1):
        ...
        self.entry1.grid(row=0, column=0, sticky='eswn',columnspan=2,pady=(10,0),padx=(10,10))
        # Lista de partes
        self.list_frame = tk.Frame(self.labelframe1)
        self.list_frame.grid(row=1, column=0,columnspan=2, sticky='ewn',pady=(10,10),padx=(10,10))
        self.list1 = tk.Listbox(self.list_frame, bd=0, bg='#8BA9CB',font=('Cambria', '14'))
        self.update_parts_list(self.part_names)
        self.update_parts_list_color()
        self.list1.pack(fill='both',side='left', expand=True)
        # Scrollbar
        self.scroll = tk.Scrollbar(self.list_frame, orient='vertical', width=25)
        self.scroll.config(command=self.list1.yview)
        self.scroll.pack(side='right', fill='y')
        ...
        self.fig_2ax_2.set_title('Selected part')
        # Crear el widget
        miniframe = tk.LabelFrame(self.frame, text='Selected part', font=('Cambria', '14'))
        miniframe.grid(row=4, column=0, sticky='ws', pady=(0, 10), padx=(0, 10), rowspan=2,columnspan=2)
        ...
        self.tableframe = tk.Label(self.frame,font=('Cambria', '16'),width=5)
        self.tableframe.grid(row=3, column=2, columnspan=2, sticky='ewn',pady=(0, 10), padx=(0, 10))
        ...
        self.entryframe = tk.LabelFrame(self.frame, text='Record point', font=('Cambria', '14'))
        self.entryframe.grid(row=4, column=2, sticky='ewns', columnspan=2, pady=(0, 10), padx=(0, 10))
        for i in range(0, 4):
        ...
        label3 = ttk.Label(self.entryframe, text='Part name:', font=('Cambria', '14'))
        label3.grid(row=0, column=0, sticky='w')
        # Nombre de la parte
        self.entry4 = tk.Button(self.entryframe, text='empty', bg='white', bd=0, width=22, anchor='w',
                                font=('Cambria', '14'))  # PARTE
        self.entry4.grid(row=0, column=1, sticky='wns')
        # Label de id
        label2 = ttk.Label(self.entryframe, text='ID:', font=('Cambria', '14'))
        label2.grid(row=0, column=3, sticky='w')
        # Boton de id
        self.entry6 = tk.Button(self.entryframe, text='empty', bg='white', bd=0, anchor='w', font=('Cambria', '14'))  # ID
        self.entry6.grid(row=0, column=3, sticky='e')
        # Label de calidad
        label = ttk.Label(self.entryframe, text='Quality:', font=('Cambria', '14'))
        ...
        # Label de Quadrante
        label4 = ttk.Label(self.entryframe, text='Quadrant:', font=('Cambria', '14'))
        label4.grid(row=1, column=3, sticky='w')
        # Boton de Quadrante
        self.entry8 = tk.Button(self.entryframe, text='empty', bg='white', bd=0, anchor='w',
                                font=('Cambria', '14'))  # Quadrant
        self.entry8.grid(row=1, column=3, sticky='e')
        # Label de problem type
        label3 = ttk.Label(self.entryframe, text='Problem Type:', font=('Cambria', '14'))
        label3.grid(row=2, column=0, sticky='w')
        problem_types = ['1ª sobremedida', '2ª sobremedida', '3A',
                             'NC', 'Quemadura', 'Delaminacion', 'Otro']
        # Boton de problem type
        self.entry7 = ttk.Combobox(self.entryframe, values=problem_types, state='readonly', width=19,font=('Cambria', '14'))
        ...
        self.button9 = ttk.Button(self.entryframe, text='Add', command=lambda: self.add_entry())
    ...
    def set_quadrant(self, event):
        ...
        if self.selected_quadrant == 'Choose a quadrant':
            return
        if self.plane_type in ['v1000', 'RPB']:
            if self.selected_quadrant == 'Left':
                self.selected_quadrant = 'Izquierdo'
        ...
        if point.part_name != '' and point.quality != '' and point.id != '' and point.problem_type != '':
            for registered_point in self.registered_points:
                if point.id == '-' and point.part_name == registered_point.part_name:
                    tk.messagebox.showerror(title='Error', message='The part has already been registered')
                    return
                if point.id == registered_point.id and point.id != '-':
                    tk.messagebox.showerror(title='Error', message='The point has already been registered')
                    return
    ...
    def delete_entry(self):
        ...
        def delete_selected_entry():
            ...
            if part_name not in part_names:
                tk.messagebox.showerror(title='Error', message='Enter a valid part name')
                return
            point_to_del = None
            row = 2
            row_number = None
        ...
        # Frame
        frame = tk.LabelFrame(delete_entry_screen, text='Delete a data recorded')
        ...
        # Boton de borrar
        button3 = ttk.Button(frame, text='Delete', command=lambda: delete_selected_entry())
    ...
    def update_parts_list_color(self):
        for point in self.registered_points:
            ...
            self.list1.itemconfig(index, bg='#429C78')
    ...
    def plot_partANDpoint(self, all_parts, part_name, clicked_point):
        for part in all_parts:
            if part.name == part_name and part_name != '-':
                print('Coordinates: ({}, {}, {})'.format(clicked_point.xe, clicked_point.ye,
                                                                           clicked_point.ze))
                x = part.x
                y = part.y
                z = part.z
                ...
                canvas_plot.get_tk_widget().pack(fill='both', side='top', expand=True)
                # Caja de informacion
                infobox = tk.LabelFrame(miniframe, text='Point information')
                infobox.pack(fill='both', side='bottom', expand=True)
                infolabel = tk.Label(infobox, text=self.point_info_text, font=('Cambria', '14'))
                infolabel.pack(fill='both')

                # Close part button
                button = ttk.Button(miniframe, text='X',
